...
Write-Host "=== StockBrief Frontend ECR Push ===" -ForegroundColor Cyan
...
docker build `
    ...
    -t "stockbrief-frontend:$IMAGE_TAG" `
    .
...
docker tag "stockbrief-frontend:$IMAGE_TAG" "${ECR_REPOSITORY}:$IMAGE_TAG"
if ($LASTEXITCODE -ne 0) {
...
if ($IMAGE_TAG -ne "latest") {
    docker tag "stockbrief-frontend:$IMAGE_TAG" "${ECR_REPOSITORY}:latest"
}
...
Write-Host "  1. ECS 서비스 업데이트 (자동 롤링 업데이트)"
Write-Host "  2. CloudFront 캐시 무효화 (필요시)"
Write-Host ""
